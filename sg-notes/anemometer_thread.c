...
float get_buffer_average(CircularBuffer* cb) {
    pthread_mutex_lock(&cb->lock);
    ...
    WeatherObs* current = cb->head;
    for (int i = 0; i < cb->size; i++) {
        buffer_sum += current->data;
        current = current->next;
    }
    pthread_mutex_unlock(&cb->lock);

    float flow_rate = (float)buffer_sum / (float)n_items;
    flow_rate = 1e6/flow_rate;
    return flow_rate;
}
...
int main(void) {
	...
	sleep(0.001);
	printf("Average: %f\n", get_buffer_average(cb));
	
    while(1)
    {
        printf("Average flow rate: %.3f\r", get_buffer_average(cb));
        sleep(0.001);
    }
